...
const DesignationManager = () => {
  const { data, error, isLoading } = useSWR(`/designation/`, fetcher, {
    errorRetryCount: 2,
  });
  ...
  return (
    <>
      <section>
        <div>
          <h2 className="border-bottom pb-2 mb-4">Manage Designation</h2>
        </div>
              ...
              {data?.length &&
                data.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <button className="border-0 rounded-1 me-2">
